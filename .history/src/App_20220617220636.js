...
class App extends React.Component {
 ...
 searchForCartProductByAttributes =(objAttributes,attributes)=>{
 ...
 searchForCartProduct =(product,attributes)=>{
  for (let i = 0; i < products.length; i++) {
    if (products[i].id === product.id) {
      ...
      if (!objAttributes.length) {
       ...
       this.searchForCartProductByAttributes(objAttributes ,attributes)
      }
    } else {
      isFound = false;
    }
  ...
  getingCartPorduct = (product) => {
    ...
    this.searchForCartProduct(product,attributes);
    return isFound;
  };
