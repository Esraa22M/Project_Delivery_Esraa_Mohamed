...
class App extends React.Component {
 ...
 searchForCartProductByAttributes =(isFound,objAttributes,attributes)=>{
 ...
 searchForCartProduct =(product,isFound ,attributes)=>{
  let products = [...this.state.itemsAddedToCart];
  for (let i = 0; i < products.length; i++) {
    if (products[i].id === product.id) {
      ...
      if (!objAttributes.length) {
       ...
       this.searchForCartProductByAttributes(isFound,objAttributes ,attributes)
      }
    } else {
      isFound = false;
    }
  ...
  getingCartPorduct = (product) => {
    ...
    this.searchForCartProduct(product,isFound,attributes);
    return isFound;
  };
