...
class Attributes extends React.Component {
  render() {
    let { product, keepButtonActive, className, attributes,details,outOfStock } = this.props;
    return (
             ...
             <div className="flex items-center" key={item.id}>{ item.items.map((item, index) => {
                return (
                  <ButtonDefault
                    ...
                    details&&outOfStock&&  keepButtonActive(item.id, getId, attributes)
                        ? "activeButton"
                        : ""
                    }`}
                    onClick={outOfStock&&details ?() =>
                  ...
                  return (
                    <SwatchContainer
                      ...
                      className={`${className[3]} ${
                       details&&outOfStock&& keepButtonActive(
                          item.id,
                          getId,
                          attributes
                        )
